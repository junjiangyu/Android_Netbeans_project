   ...
   private Spinner spinner;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
        ...
        regBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    pwdinput = hash(pwd.getText().toString());
    ...
    private class PostAsyncTask extends AsyncTask<String, Void, String>
    {
        ...
        protected String doInBackground(String... params) {
            String fnvalue = RestClient.login(usernameinput);
            String emailreturn = RestClient.email(emailinput);
            //check the return value if it is empty,if it is empty then account not duplicate
            if (!fnvalue.equals("[]") | !emailreturn.equals("[]"))
            {
            return ("There is already an account exist! Try another user name or email");
            }
            else {
                ...
                RestClient.createAccount(user, usernameinput, pwdinput);
      ...
      public String hash(String password) throws NoSuchAlgorithmException {
          MessageDigest hashMethod = null;
          hashMethod = MessageDigest.getInstance("MD5");
          hashMethod.update(password.getBytes());

          byte[] data = hashMethod.digest();
          StringBuffer hashdata = new StringBuffer();

          for (int i = 0; i < data.length; i++){

              hashdata.append(Integer.toHexString(0xFF & data[i]));
          }
